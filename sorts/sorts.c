...
void report(struct SortAlg *alg_ptr, const size_t length)
{
  ...
  line_width = (col_width * num_cols) + pad_gutter - 2;
  seg_bytes  = (line_width * 3);
  top_line = (char *) malloc(seg_bytes + 1);
  mid_line     = (char *) malloc(seg_bytes + 1);
  byte_index = 0;

  while (byte_index < seg_bytes) {
    line[byte_index] = 0xe2;
    ++byte_index;
    line[byte_index] = 0x95;
    ++byte_index;
    line[byte_index] = 0x90;
    ++byte_index;
  }

  line[byte_index] = 0x00;

  /* clear window => inverse ANSI => print top border */
  printf("\e[2J\e[7m╔%s╗\n║ %-*s ", line, col_width, "sorting algorithm");
