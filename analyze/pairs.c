...
DO_RECURSE:
	...
	if (pivot_nut->next != &last_nut)
		do_match(pivot_nut->next,  last_nut.prev,
			 pivot_blt->next,  last_blt.prev);

	/* match nodes to the left of pivot */
	PRINT_LABEL("DO LEFT");
	if (pivot_nut->prev != &head_nut)
		do_match(head_nut.next, pivot_nut->prev,
			 head_blt.next, pivot_blt->prev);
	...
	head_nut.next->prev = head_nut.prev;
	if (head_nut.prev != NULL)
		head_nut.prev->next = head_nut.next;

	last_nut.prev->next = last_nut.next;
	if (last_nut.next != NULL)
		last_nut.next->prev = last_nut.prev;

	head_blt.next->prev = head_blt.prev;
	if (head_blt.prev != NULL)
		head_blt.prev->next = head_blt.next;

	last_blt.prev->next = last_blt.next;
	if (last_blt.next != NULL)
		last_blt.next->prev = last_blt.prev;
}
				 ...
				 const size_t count)
{
	...
	struct DblNode *next_nut = &prev_nut[i_last];
	struct DblNode *next_blt = &prev_blt[i_last];


	struct DblNode head_nut = { .value = -999.999, .prev = NULL, .next = prev_blt };
	struct DblNode head_blt = { .value = -999.999, .prev = NULL, .next = prev_blt };
	struct DblNode last_nut = { .value =  999.999, .prev = next_blt, .next = NULL };
	struct DblNode last_blt = { .value =  999.999, .prev = next_blt, .next = NULL };
	...
	head_nut.next->prev = NULL;
	head_blt.next->prev = NULL;
	last_nut.prev->next = NULL;
	last_blt.prev->next = NULL;

	*matched_nuts = head_nut.next;
	*matched_blts = head_blt.next;
}
